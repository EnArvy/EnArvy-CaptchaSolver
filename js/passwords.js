...
function changepass(field, trypass, newpass) {
  if (trypass == "") {
    ...
    chrome.storage.local.get([field], function (result) {
      if (
        CryptoJS.AES.decrypt(result[field], "function()").toString(
          CryptoJS.enc.Utf8
        ) == trypass
      ) {
        var dataObj = {};
...
function change(field, pass, value) {
  chrome.storage.local.get([field, "master"], function (result) {
    if (
      CryptoJS.AES.decrypt(result.master, "function()").toString(
        CryptoJS.enc.Utf8
      ) == pass
    ) {
      var dataObj = {};
...
function fill(ufield, uid, pfield, pid, cap, capid, bid) {
  chrome.storage.local.get([uid, pid], function (result) {
    document.getElementById(ufield).value = CryptoJS.AES.decrypt(
      result[uid],
      "function()"
    ).toString(CryptoJS.enc.Utf8);
    document.getElementById(pfield).value = CryptoJS.AES.decrypt(
      result[pid],
      "function()"
    ).toString(CryptoJS.enc.Utf8);
...
function kerfill(ufield, uid, pfield, pid, cap, capid, bid) {
  chrome.storage.local.get([uid, pid], function (result) {
    document.getElementsByName(ufield)[0].value = CryptoJS.AES.decrypt(
      result[uid],
      "function()"
    ).toString(CryptoJS.enc.Utf8);
    document.getElementsByName(pfield)[0].value = CryptoJS.AES.decrypt(
      result[pid],
      "function()"
    ).toString(CryptoJS.enc.Utf8);
